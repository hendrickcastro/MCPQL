...
import {
    mcp_table_analysis,
    mcp_get_columns,
    mcp_get_primary_keys,
    mcp_get_foreign_keys,
    mcp_get_indexes,
    mcp_get_constraints,
...
import {
    mcp_sp_structure,
    mcp_get_sp_info,
    mcp_get_sp_parameters,
    mcp_get_sp_dependencies,
    mcp_get_sp_definition,
    mcp_get_sp_complete_structure,
    mcp_get_sp_definition_simple,
    mcp_get_sp_parameters_simple,
    mcp_get_sp_dependencies_simple,
    ...
    describe('📊 Individual Table Analysis Functions - Python Style', () => {
        ...
        it('should get detailed column information', async () => {
            ...
            if (result.success) {
                ...
                const columnNames = result.data.map((col: any) => col.column_name);
        ...
        it('should get constraint information', async () => {
            ...
            if (result.success) {
                ...
                if (result.data.length > 0) {
                    ...
                    result.data.forEach((constraint: any) => {
                        table.push([
            ...
            if (result.success) {
                ...
                if (columns && columns.length > 0) {
                    ...
                    columns.forEach((col: any) => {
                        const isPK = primary_keys.some((pk: any) => pk.column_name === col.column_name);
        ...
        it('should analyze stored procedure structure', async () => {
            ...
            if (result.success) {
                ...
                if (result.data.parameters.length > 0) {
                    ...
                    result.data.parameters.forEach((param: any) => {
                        table.push([
                ...
                if (result.data.dependencies && result.data.dependencies.length > 0) {
                    ...
                    result.data.dependencies.forEach((dep: any) => {
                        table.push([
                            dep.referenced_schema_name || 'N/A',
                            dep.referenced_entity_name || 'N/A',
        ...
        it('should get complete stored procedure structure', async () => {
            ...
            if (result.success) {
                ...
                if (result.data.info) {
                    ...
                    summaryTable.push(
                        ['Schema', result.data.info.schema_name || 'N/A'],
                        ['Name', result.data.info.procedure_name || 'N/A'],
                        ['Type', result.data.info.type_desc || 'N/A'],
                        ['Parameters', result.data.parameters.length.toString()],
                        ['Dependencies', result.data.dependencies.length.toString()],
                        ['Definition Length', result.data.info.definition_length?.toString() || 'N/A'],
                        ['Created', result.data.info.create_date || 'N/A'],
                        ['Modified', result.data.info.modify_date || 'N/A']
                    );
    ...
    describe('⚙️ Stored Procedure Info - Simple Functions (Python StoredProcedureInfo Style)', () => {
        ...
        it('should get all stored procedure info (simple - matching Python get_all_info)', async () => {
            ...
            if (result.success) {
                ...
                summaryTable.push(
                    ['Definition Available', result.data.definition ? 'Yes' : 'No'],
                    ['Definition Length', result.data.definition ? result.data.definition.length.toString() : '0'],
                    ['Parameters Count', result.data.parameters.length.toString()],
                    ['Dependencies Count', result.data.dependencies.length.toString()]
                );
        ...
        describe('📄 Search in Definitions', () => {
            test('🔍 mcp_search_in_definitions - should find objects by content', async () => {
                ...
                if (result.success && result.data && result.data.length > 0) {
                    ...
                    result.data.slice(0, 5).forEach((obj: any) => { // Show first 5
                        table.push([
                            chalk.yellow(obj.object_name),
                            chalk.blue(obj.object_type),
                            chalk.green(obj.schema_name || 'dbo'),
        ...
        describe('📋 Search by Type', () => {
            test('🗂️ mcp_search_objects_by_type - should find all objects of specific type', async () => {
                ...
                if (result.success && result.data && result.data.length > 0) {
                    ...
                    result.data.slice(0, 10).forEach((obj: any) => { // Show first 10 tables
                        table.push([
                            chalk.yellow(obj.object_name),
                            chalk.green(obj.schema_name || 'dbo'),
                            chalk.gray(obj.create_date ? new Date(obj.create_date).toLocaleDateString() : 'N/A'),
            ...
            test('🚀 mcp_search_comprehensive - should perform complete search', async () => {
                ...
                if (result.success && result.data) {
                    ...
                    if (result.data.name_matches.length > 0) {
                        ...
                        result.data.name_matches.slice(0, 3).forEach((obj: any) => {
                            nameTable.push([
                                chalk.yellow(obj.object_name),
                                chalk.blue(obj.object_type),
                                chalk.green(obj.schema_name || 'dbo')
            ...
            test('🚀 mcp_quick_data_analysis - should perform comprehensive analysis', async () => {
                ...
                if (result.success && result.data) {
                    ...
                    if (result.data.columns_info.length > 0) {
                        ...
                        result.data.columns_info.forEach((col: any) => {
                            columnsTable.push([
                    ...
                    if (result.data.preview_data.length > 0) {
                        ...
                        result.data.preview_data.forEach((row: any) => {
                            previewTable.push(Object.values(row).map((value: any) =>
                                chalk.white(String(value).substring(0, 20) + (String(value).length > 20 ? '...' : ''))
                            ));
                        });
    ...
    describe('⚙️ Stored Procedure Execution', () => {
        test('🚀 mcp_execute_procedure - should execute stored procedure', async () => {
            ...
            if (spSearchResult.success && spSearchResult.data && spSearchResult.data.length > 0) {
                ...
                const simpleProcedures = spSearchResult.data.filter((sp: any) =>
                    sp.object_name &&
