...
type AuthenticationType = 'sql' | 'windows' | 'azure-ad';
...
const buildServerString = (): string => {
  const server = process.env.DB_SERVER as string;
...
const buildConfig = (): sql.config => {
  const authenticationType = (process.env.DB_AUTHENTICATION_TYPE || 'sql').toLowerCase() as AuthenticationType;
...
const getConnectionConfig = (): { config?: sql.config; connectionString?: string } => {
