 ...
 */
export const mcp_get_all_table_info = async (args: { table_name: string }): Promise<ToolResult<{
  columns: any[];
  primary_keys: any[];
  foreign_keys: any[];
  indexes: any[];
  ...
  try {
    ...
    const [columnsResult, primaryKeysResult, foreignKeysResult, indexesResult, constraintsResult] = 
      await Promise.all([
    ...
    if (!primaryKeysResult.success) return primaryKeysResult;
    if (!foreignKeysResult.success) return foreignKeysResult;
    if (!indexesResult.success) return indexesResult;
    if (!constraintsResult.success) return constraintsResult;
